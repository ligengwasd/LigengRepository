...
public class FindServices {
    ...
    public static Map<String, Object> performFindTableDirOptions(DispatchContext dctx, Map<String, ?> context) {
    	Map<String, Object> result = ServiceUtil.returnSuccess();
    	
    	Delegator delegator = dctx.getDelegator();
    	Map inputFields = FastMap.newInstance();
    	inputFields = JSONObject.fromObject((String)context.get("inputFields"));  
    	String entityName = (String)context.get("entityName");
    	String nameField = (String)context.get("nameField");
    	String valueField = (String)context.get("valueField");
    	String conditionJson = (String)context.get("conditionJson");
    	
    	String orderBy = new String();
    	String sidx = (String)context.get("sidx");
    	String sord = (String)context.get("sord");
    	
    	if(!UtilValidate.isEmpty(sidx)){
    		orderBy = sidx;
    		...
    		if(!UtilValidate.isEmpty(entityName)){
    			...
    			for(int i=0;i<entity.getPkFieldNames().size();i++){
    				String temp = entity.getPkFieldNames().get(i);
    				if(i==0){
    					orderBy = temp;
    				}else{
    	...
    	Set<String> fieldSet = UtilMisc.toSet(nameField,valueField) ;
    	EntityCondition mainCond = null;
    	List<EntityCondition> andExprs = FastList.newInstance();
    	
    	for(Object entryName :  inputFields.entrySet()){
    		andExprs.add(EntityCondition.makeCondition(entryName, EntityOperator.EQUALS, inputFields.get(entryName)));
    	}
    	//添加查询条件
    	if (!conditionJson.equals("") &&  !conditionJson.equals("undefined")) {
    		JSONObject jsonObject = JSONObject.fromObject(conditionJson);
    		JSONArray  conditionJsonArray = jsonObject.getJSONArray("data");
    		for (int i = 0; i < conditionJsonArray.size(); i++) {
    			andExprs.add(EntityCondition.makeCondition(conditionJsonArray.getJSONObject(i).get("conditionField") ,EntityOperator.EQUALS,conditionJsonArray.getJSONObject(i).get("conditionFieldValue")));
			}
		}
    	...
    	str.append("[");
    	try {
    		entityList = delegator.findList(entityName, mainCond, fieldSet,UtilMisc.toList(orderBy), null, false);
    		int i =0;
    		str.append("{\"value\":\"");
			str.append("");
			str.append("\"");
    		...
    		for(GenericValue entity : entityList){
    			...
    			str.append("{\"value\":\"");
    			str.append(entity.get(valueField));
    			str.append("\"");
    			str.append(",");
    			str.append("\"text\":\"");
    			str.append(entity.get(nameField));
    			str.append("\"}");
    			i++;
    		}
    		str.append("]");
    	...
    	result.put("editoptions", str.toString());
    	//System.out.println(str.toString());
    	return result;
    }
    
    public static Map<String, Object> performFindKendoGrid(DispatchContext dctx, Map<String, ?> context) {
    	Map<String, Object> result = ServiceUtil.returnSuccess();
    	...
    	if((!UtilValidate.isEmpty(sidx))&&(!UtilValidate.isEmpty(sord))){
    		orderBy = sidx;
    		if(sord.equalsIgnoreCase("desc")){
    			orderBy = "-"+orderBy;
    		}
    		...
    		if(!UtilValidate.isEmpty(entityName)){
    			...
    			ModelEntity entity = delegator.getModelEntity(entityName);
    			int i = 0;
    			for( String temp : entity.getPkFieldNames()){
    				if(i==0){
    					orderBy = temp;
    				}else{
    					orderBy = orderBy+ "|" + temp;
    	...
    	try {
    		viewSize = Integer.parseInt((String) context.get("pageSize"));
    	} catch (Exception e) {
    		viewSize = 10;
    	}
    	
    	try {
    		Long page = Long.parseLong(context.get("page").toString());
    		viewIndex = (page.intValue()-1)*viewSize;
    	} catch (Exception e) {
    		viewIndex = 0;
    	}
    	...
    	fieldMap.put("orderBy", orderBy);
    	fieldMap.put("inputFields", inputFields);
    	fieldMap.put("fieldList", fieldList);
    	fieldMap.put("userLogin", (GenericValue)context.get("userLogin"));
    	
    	fieldMap.put("timeZone", (TimeZone)context.get("timeZone"));
    	fieldMap.put("filterByDate",(String) context.get("filterByDate"));
    	fieldMap.put("filterByDateValue",(Timestamp) context.get("filterByDateValue"));
    	fieldMap.put("locale", (Locale) context.get("locale"));
    	fieldMap.put("viewIndex", viewIndex);
    	fieldMap.put("viewSize", viewSize);
    	try {
    		executeResult = dispatcher.runSync("performFind",fieldMap);
    		//System.out.println("executeResult::"+executeResult);
    	} catch (Exception e) {
    		e.printStackTrace();
    		return ServiceUtil.returnError("ddddds");
    	}
    	List<GenericValue> listIt = FastList.newInstance();
    	EntityListIterator entityList = null;
    	try {
    		entityList = (EntityListIterator)executeResult.get("listIt");
    		//System.out.println("entityList::"+ entityList);
    		if(entityList!=null){
    			listIt = entityList.getCompleteList();
    			//System.out.println("listIt::"+listIt);
    			listIt = listIt.subList(viewIndex, Math.min( viewIndex+viewSize,listIt.size()));
    			//System.out.println("listIt::"+listIt);
    		}
    		entityList.close();
    	} catch (GenericEntityException e) {
...
//			}
    	...
    	Integer listSize = (Integer)executeResult.get("listSize");
    	//BigDecimal total = new BigDecimal(listSize).divide(new BigDecimal(viewSize),BigDecimal.ROUND_UP);
    	//result.put("page", (String)context.get("page"));
    	...
    	result.put("total", listSize.longValue());
//    	System.out.println(JSONArray.fromObject(JSONArray.fromObject(listIt,jsonConfig).toString()));
    	result.put("rows",JSONArray.fromObject(JSONArray.fromObject(listIt,jsonConfig).toString().replace(":null", ":\"\""))  );
    	return result;
    }
