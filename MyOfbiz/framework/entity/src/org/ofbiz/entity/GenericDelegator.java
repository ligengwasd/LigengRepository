...
public class GenericDelegator implements Delegator {
    ...
    public Long getNextLineNo(String seqEntityName) {
    	return getNextLineNo(seqEntityName,null);
    }
    
    public Long getNextLineNo(String seqEntityName,String refRecordId) {
    	return getNextLineNo(seqEntityName,refRecordId,10);
    }
    
    public Long getNextLineNo(String seqEntityName,String refRecordId, long staggerMax) {
        boolean beganTransaction = false;
        try {
            if (alwaysUseTransaction) {
                beganTransaction = TransactionUtil.begin();
            ...
            Map<String,String> fields = UtilMisc.toMap("seqEntityName", seqEntityName,"refRecordId",refRecordId);
			List<GenericValue> adSequenceList = this.findByAnd("AdSequence", fields, null, false);
			GenericValue adSequence;
			if(adSequenceList.size()==0){
				String adSequenceId = this.getNextSeqId("AdSequence");
				adSequence = this.makeValue("AdSequence");
				adSequence.set("adSequenceId", adSequenceId);
				adSequence.set("seqEntityName", seqEntityName);
				adSequence.set("refRecordId", refRecordId);
				adSequence.set("startno", 10L);
				adSequence.set("currentnext", 10L);
				adSequence.create();
			}else{
				adSequence = adSequenceList.get(0);
				Long currentnext = (Long)adSequence.get("currentnext");
				adSequence.set("currentnext", currentnext+staggerMax);
				adSequence.store();
			}
            return (Long)adSequence.get("currentnext");
        } catch (GenericEntityException e) {
            String errMsg = "Failure in getNextSeqIdLong operation for seqName [" + seqEntityName + "]: " + e.toString() + ". Rolling back transaction.";
            try {
                TransactionUtil.rollback(beganTransaction, errMsg, e);
            } catch (GenericEntityException e2) {
                Debug.logError(e2, "[GenericDelegator] Could not rollback transaction: " + e2.toString(), module);
