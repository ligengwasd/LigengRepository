...
<#macro renderDisplayField type imageLocation idName description title class alert inPlaceEditorUrl="" inPlaceEditorParams="">
  ...
  <#else>
    <#if inPlaceEditorUrl?has_content || class?has_content || alert=="true" || title?has_content>
      <span <#if idName?has_content>id="cc_${idName}"</#if> 
      		<#if title?has_content>title="${title}"</#if> 
      		<@renderClass class alert />
      ><#t/>
    </#if>
...
<#macro renderButtonListOpen>
<tr>
<td></td>
<td>
</#macro>
<#macro renderButtonListClose>
</td></tr>
</#macro>
<!--kd-Input-->
<#macro kdInput id name type onclick style value ajaxUrl updateGridName>
	<input style="width:80px;" <#if id?has_content>id="${id}"</#if>
		<#if name?has_content>name="${name}"</#if>
		class="${style}"  type="${type}" value = "${value}" 
		<#if updateGridName?has_content>onclick="${updateGridName}_Refresh(this)"</#if>
		<#if onclick?has_content>onclick="${onclick}"</#if>
		<#if ajaxUrl?has_content>onclick="fun1(this,'${ajaxUrl}')"</#if>
		
	/>
</#macro>
<!-- kdDatepicker start by ligeng -->
<#macro kdDatePicker id name format max min start depth defaultValue>
	<input id="${id}" name="${name}"  style="width:150px;height:25px"/>	
	<script language="javascript">
            $(document).ready(function() {
                $("#${id}").kendoDatePicker({
                	<#if format?has_content>format:"${format}",<#else>format:"yyyy-MM-dd",</#if> 
                	<#if max?has_content>max:new Date("${max}"),</#if>
                	<#if min?has_content>min:new Date("${min}"),</#if>
                	<#if start?has_content>start:"${start}",</#if>
                	<#if depth?has_content>depth:"${depth}",</#if>
...
<!-- kdDatepicker end by ligeng -->

<!-- kdTimePicker start by ligeng -->
<#macro kdTimePicker id name format max min defaultValue>
	<input id="${id}" name="${name}"  style="width:150px;height:25px"/>	
	<script language="javascript">
            $(document).ready(function() {
                $("#${id}").kendoTimePicker({
                	<#if format?has_content>format:"${format}",<#else>format:"HH:mm:ss",</#if> 
                	<#if max?has_content>max:new Date("${max}"),</#if>
                	<#if min?has_content>min:new Date("${min}"),</#if>
                	<#if defaultValue?has_content>value:"${defaultValue}"</#if> 
                });
...
<!-- kdTimePicker end by ligeng -->

<!-- kdDateTimepicker start by ligeng -->
<#macro kdDateTimePicker id name format max min defaultValue>
	<input id="${id}"  name="${name}" style="width:230px;height:25px"/>
	<script language="javascript">
            $(document).ready(function () {
                $("#${id}").kendoDateTimePicker({
                	<#if format?has_content>format:"${format}",<#else>format:"yyyy-MM-dd HH:mm:ss",</#if> 
                	<#if max?has_content>max:new Date("${max}"),</#if>
                	<#if min?has_content>min:new Date("${min}"),</#if> 
                    <#if defaultValue?has_content>value:"${defaultValue}"</#if> 
                });
...
<!-- kdDateTimepicker end by ligeng -->

<!-- kdAutoComplete start by ligeng -->
<#macro kdAutoComplete id name entityName nameField valueField delay height minLength placeholder>
	<input id="${id}" name="${name}" />
	<script>
		var data_${id} = findTableDirOptions('${entityName}','${nameField}','${valueField}');
...
<!-- kdautocomplete end by ligeng -->

<!-- kdDropDownList start by ligeng -->
<#macro kdDropDownList id name entity type refenceValue nameField valueField delay height optionLabel value conditionName conditionValue>
	<input id="${id}" name="${name}" />
	<script>
		var data_${id};
		<#if "${type}"=="tableDir">
			data_${id} = findTableDirOptions('${entity}','${nameField}','${valueField}','${conditionName}','${conditionValue}');
		<#else>
...
<!-- kdNumericTextBox start by ligeng -->
<#macro kdNumericTextBox id name format culture decimals max min placeholder value step>
	<input id="${id}" name = "${name}" style="width:150px;height:25px"/>
	<script>
		$("#${id}").kendoNumericTextBox({
			<#if format?has_content>format:"${format}",</#if>
		  	<#if culture?has_content>culture:"${culture}",</#if>
		 	<#if decimals?has_content>decimals:"${decimals}",</#if>
...
<!-- kdNumericTextBox end by ligeng -->
...
<#macro renderCheckField items className alert id allChecked currentValue name event action>
  <#list items as item>
    <span <@renderClass className alert />><#rt/>
      <input type="checkbox" onchange="this.value=(this.checked?'Y':'N')" <#if (item_index == 0)> id="${id}"</#if><#rt/>
